...
//~ @action scroll to hash ( when the windows open )
...
//~ @function scroll to hash
function dmengFixedSidebarLastChild(){
	if( $('#sidebar').length<=0 || $('#content').height() <= $('#sidebar').height() ) return;
	var rs = arguments[0] ? arguments[0] : 0;
	...
	if( w>991 ){
		...
		var c = $('#colophon');
		if( ( st + c.outerHeight(true) + parseInt(c.css('margin-top')) + e.outerHeight(true) ) >= dh ) e.css({'position':'absolute','top':c.offset().top - parseInt(c.css('margin-top')) - e.outerHeight(true) - t });
	}
...
function dmeng_get_comments(c,m,e){
	$(e).html('<div id="loading"><span class="glyphicon glyphicon-refresh"></span></div>');
	$.ajax({
		type: 'POST', 
		url: ajaxurl, 
		data: {
...
$(document).on("submit","#commentform",function(){
		$('#commentsubmit').addClass('disabled').append('<span class="glyphicon glyphicon-refresh"></span>');
...
function dmeng_set_body_padding_top(){
	header = $('#masthead');
	if(header.hasClass('navbar-fixed-top')){
		$('body').css('padding-top',parseInt(header.height())+20);
	}
}
//~ @jquery ready
$(document).ready(function(){
	// body padding top
	dmeng_set_body_padding_top();
	// fixed sidebar last child
	dmengFixedSidebarLastChild();
