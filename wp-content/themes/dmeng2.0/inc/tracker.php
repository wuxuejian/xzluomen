 ...
 */
 function dmeng_tracker_rank($type='single',$limit=10){
	global $wpdb;
	$table_name = $wpdb->prefix . 'dmeng_tracker';
	$type = in_array($type,array('single','search')) ? $type : 'single';
	return $wpdb->get_results( "SELECT pid,traffic FROM $table_name WHERE type = '$type' ORDER BY -traffic ASC LIMIT $limit ");
}
