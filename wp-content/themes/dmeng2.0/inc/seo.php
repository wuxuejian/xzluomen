...
function dmeng_wp_title( $title, $sep ) {
	...
	if( is_attachment() ) $title .= get_post_mime_type()." $sep " ;

	if( is_category() ) $title .= __('分类目录','dmeng')." $sep " ;
	
	if( is_tag() ) $title .= __('标签','dmeng')." $sep " ;
	
	if( is_author() ) $title .= __('作者','dmeng')." $sep " ;

	if( is_archive() ) $title .= __('归档','dmeng')." $sep " ;
	
	if( is_search() ) $title .= sprintf(__('%s个相关结果','dmeng'), $wp_query->found_posts )." $sep " ;
