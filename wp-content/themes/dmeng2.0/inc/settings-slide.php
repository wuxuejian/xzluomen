...
function dmeng_options_slide_page(){
  ...
  if( isset($_POST['action']) && wp_verify_nonce( trim($_POST['_wpnonce']), 'check-nonce' ) ) :
	...
	?>
<style>
#slide-list li{font-size: 13px;line-height:36px;color: #777;background:#fff;border:1px solid #e1e1e1;padding-left:12px;}
...
//~ delete_option('dmeng_slide_data');
...
<ul id="slide-list">
	<li>
		<?php printf( __('现在有 %1$s 组幻灯片，你可以选择管理或 <a href="%2$s">新建幻灯片</a>。','dmeng'), count($slides), add_query_arg(array('do'=>'edit','id'=>1)));?>
...
<?php
	...
	<li>
		...
		<span><?php _e('操作','dmeng');?> : <a href="<?php echo add_query_arg(array('do'=>'edit','id'=>$slide_id));?>"><?php _e('编辑','dmeng');?></a> | <a href="<?php echo add_query_arg(array('do'=>'delete','id'=>$slide_id,'_wpnonce'=>wp_create_nonce( 'check_for_'.$slide_id )));?>" title="<?php _e('删除','dmeng');?><?php echo $slide->name;?>" class="confirm_slide yellow"><?php _e('删除','dmeng');?></a></span>
	
	<?php if( $slide_home && $slide_id == $slide_home ) { ?>
		<span><a href="<?php echo add_query_arg(array('do'=>'rfhome','id'=>$slide_id,'_wpnonce'=>wp_create_nonce( 'check_for_'.$slide_id )));?>" title="<?php _e('取消首页幻灯片','dmeng');?>" class="confirm_slide red"><?php _e('取消首页幻灯片','dmeng');?></a></span>
	<?php }else{ ?>
		<span><a href="<?php echo add_query_arg(array('do'=>'home','id'=>$slide_id,'_wpnonce'=>wp_create_nonce( 'check_for_'.$slide_id )));?>" title="<?php _e('设为首页幻灯片','dmeng');?>" class="confirm_slide"><?php _e('设为首页幻灯片','dmeng');?></a></span>
	<?php } ?>
