...
function get_dmeng_message( $uid=0 , $count=0, $where='', $limit=0, $offset=0 ){
	...
	if($count){
		...
		$check = $wpdb->get_results( "SELECT msg_id,msg_type,msg_date,msg_title,msg_content FROM $table_name WHERE msg_type!='credit' AND user_id='$uid' ORDER BY msg_date DESC LIMIT $offset,$limit" );
	}
