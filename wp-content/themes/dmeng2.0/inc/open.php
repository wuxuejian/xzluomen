...
<ul class="dmeng-open">
	<li>
		<a class="qq" href="<?php echo home_url('/?connect=qq&action=login&redirect='.urlencode(dmeng_get_redirect_uri()));?>" title="使用QQ账号登录">使用QQ账号登录</a> 
	</li>
	<li>
		<a class="weibo" href="<?php echo home_url('/?connect=weibo&action=login&redirect='.urlencode(dmeng_get_redirect_uri()));?>" title="使用微博账号登录">使用微博账号登录</a>
	</li>
</ul>
	<?php
}
...
function dmeng_open_template_redirect(){
			...
			if(is_user_logged_in()){
				...
				if( is_wp_error($insert_user_id) ) {
					...
					if( $is_new_user ){
						...
						wp_update_user( array ('ID' => $insert_user_id, 'role' => 'contributor' ) );
						add_dmeng_message( $insert_user_id, 'unread', current_time('mysql'), __('请完善账号信息','dmeng'), sprintf(__('欢迎来到%1$s，请<a href="%2$s">完善资料</a>，其中电子邮件尤为重要，许多信息都将通过电子邮件通知您！','dmeng') , get_bloginfo('name'), dmeng_get_user_url('profile')) );
					}
	...
	if( isset($_GET['connect']) && trim($_GET['connect'])==='qq' && dmeng_is_open_qq() && ( is_home() ||is_front_page() ) ){
		...
		if( isset($_GET['code']) && isset($_GET['state']) && isset($_COOKIE['qq_state']) && $_COOKIE['qq_state']==md5($_GET['state']) ){
			...
			$response = dmeng_get_url('https://graph.qq.com/oauth2.0/token?'.http_build_query($params));
			...
			$info = json_decode(dmeng_get_url($info_url));
	...
	if( isset($_GET['connect']) && trim($_GET['connect'])==='weibo' && dmeng_is_open_weibo() && ( is_home() ||is_front_page() ) ){
		...
		if( isset($_GET['code']) ){
			...
			$access = dmeng_get_url('https://api.weibo.com/oauth2/access_token?',http_build_query(array(
				'grant_type'=>'authorization_code',
				'client_id'=>$OPEN_WEIBO['KEY'],
			...
			$info = dmeng_get_url('https://api.weibo.com/2/users/show.json?access_token='.$token.'&uid='.$openid);
