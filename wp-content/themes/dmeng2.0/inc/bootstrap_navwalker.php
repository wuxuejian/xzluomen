...
class Dmeng_Bootstrap_Menu extends Walker {
	...
	function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		if ( in_array( 'current-menu-item', $item->classes ) || (  $depth == 0 && (in_array( 'current-menu-parent', $item->classes ) || in_array( 'current-menu-ancestor', $item->classes ) ) ) ){
			$class_names[] = 'active';
		}
		
		$class_names = $class_names ? ' class="' . esc_attr( join(' ',$class_names) ) . '"' : '';
